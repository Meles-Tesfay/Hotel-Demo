    ...
    return (
        ...
        >
            <div className="relative overflow-hidden h-52">
                ...
                <div className={`absolute top-3 right-3 bg-gradient-to-r ${accent.badge} px-3 py-1 rounded-full text-xs font-bold text-white shadow-lg`}>
                    {item.category}
                </div>
                ...
                <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-xl shadow-md opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
                    <span className="font-bold text-gray-900 text-sm">{currency}{item.price}</span>
                </div>
            </div>

            <div className='p-5'>
                <div className='flex items-start justify-between mb-2'>
                    <p className='font-playfair text-lg font-bold text-gray-900 leading-tight line-clamp-1'>{item.title}</p>
                </div>
                ...
                <div className='flex items-center gap-1.5 text-xs text-gray-400 mb-4'>
                    ...
                    <span className="truncate">{item.hotel?.name}</span>
                </div>

                <div className='flex items-center justify-between'>
                    <div>
                        <span className='text-2xl font-bold text-gray-900'>{currency}{item.price}</span>
                    </div>
